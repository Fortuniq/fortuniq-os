...
import { ShieldCheck, LogIn, UserCog, UserPlus, UserMinus, FileEdit, FileText, Eye } from "lucide-react";
...
export function AuditView({ entries, roleCounts }: { entries: AuditEntry[]; roleCounts: { signIns: number; permissionChanges: number; documentViews: number } }) {
  return (
    <div>
      <PageHeader
        title="Audit Logs"
        description="A record of sign-ins, permission changes, and document activity across FortunIQ OS."
      />
      ...
      <p className="text-[11px] text-light-grey mt-4">
        Visible only to Super Admin and HR/Admin roles. See docs/AUDIT_LOGS.md for exactly what is and isn&apos;t
        currently recorded.
      </p>
    </div>
  );
