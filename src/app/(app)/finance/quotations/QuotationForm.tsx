...
import { createQuotationDraft, updateQuotationDraft } from "./quotation-actions";
import { formatZARFull } from "@/lib/format";
import type { CustomerPickerOption } from "@/lib/quotations-data";
...
/**
 * Client-side arithmetic here is a PREVIEW ONLY, so the person sees a
 * running total as they type — it does not decide what gets stored.
 * The authoritative calculation (and the VAT hard-block decision) runs
 * server-side in finance-core.ts / quotation-actions.ts when the form
 * is submitted. See docs/FINANCE_MODULE.md.
 */
...
export function QuotationForm({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = mode === "edit" ? await updateQuotationDraft(formData) : await createQuotationDraft(formData);
      if (result?.error) {
        setError(result.error);
        return;
      }
      const targetId = mode === "edit" ? initial!.id : (result as { id?: string }).id;
      router.push(targetId ? `/finance/quotations/${targetId}` : "/finance/quotations");
      router.refresh();
    });
  }
  ...
  if (customers.length === 0) {
    return (
      <div className="bg-white rounded-xl border border-border p-6 text-sm text-grey">
        You aren&apos;t authorised to create a quotation for any customer yet. If you&apos;re a Sales user, ask your manager to assign you an account in Customers, or contact Finance.
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-border p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div>
          <label className="text-xs font-medium text-grey block mb-1">Valid Until (optional)</label>
          <input type="date" value={validUntil ?? ""} onChange={(e) => setValidUntil(e.target.value)} className="w-full text-sm px-3 py-2 rounded-lg border border-border" />
        </div>
      </div>
